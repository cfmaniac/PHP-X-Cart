...
ajax.widgets.checkout.obj.prototype.doUpdateProfile = function(e) { 
  ...
  /* start_ch_mod */
  /*----------------------------------------------------------------------------*
  	Additional Cooper's Hawk Functionality
	If the cart has alcoholic products in it verify the customers age.
		Alerts the customer if they are not logged in or not 21 or older.
  	Written by: Peter Juszczyk 2013
  *----------------------------------------------------------------------------*/
  
  // get the customers birthday as a string
  var bday_string = $('#additional_values_1').val();
  
  // check that the date provided is valid
  if (!isValidDate(bday_string)){
	var msg = 'The date you have entered is not valid. Please enter the date in mm/dd/yyyy format.';
	xAlert(msg,'','W');
	return true;
  }
  
  // if the cart has alcohol check that the person is 21 or older
  if ($('#cart_has_alcohol').val() == 'Y'){
	  // check if the person is over 21 (and under 120)
	  var isValidAge = bday_string == "" ? false : validateAge(bday_string);
	  if (!isValidAge){
	  	var msg = 'There is a product containing alcohol in your shopping cart. You must be 21 or older to continue. Please update your information or edit your cart contents.';
  		xAlert(msg,'','W');
		return true;
	  }
  }
  
  // update hidden form fields used for checking compliance
  $('#h_firstname').val($('#firstname').val());
  $('#h_lastname').val($('#lastname').val());
  $('#u_birthday').val($('#additional_values_1').val());
  var memberNumber = $('#additional_values_2').val();
  if(memberNumber != ""){
	 $('#u_membership').val("Wine Club Member");
  } else {
	 $('#u_membership').val("");
  }
  ...
  var prefix = '#b_';
  var suffix = 'B';
  var is_checked = $('#ship2diff').prop('checked');
  ...
  var address_2 = $(prefix+'address_2').val()
  var city = $(prefix+'city').val()
  var state = $('#address_book_'+suffix+'_state').val()
  var country = $(prefix+'country').val()
  var zipcode = $(prefix+'zipcode').val()
  
  $('#h_s_address').val(address);
  $('#h_s_address_2').val(address_2);
  $('#h_s_city').val(city);
  $('#h_s_state').val(state);
  $('#h_s_country').val(country);
  $('#h_s_zipcode').val(zipcode);

  stination ="/ship_compliant_shop_client.php";
  doAjax("#compliance_form", destination, "checkComplianceSuccess", "", "", "post", false);
  
  // if order is not compliant
  if ($('// check that the order is compliant
  var de#is_compliant').val() == 'N'){
	  return true;
  }
...
ajax.widgets.checkout.obj.prototype.changeProfile = function() {
  ...
  return ajax.core.loadBlock(this.profile, 'opc_profile', { edit_profile: true }, function() {
  	...
  	$("#additional_values_1").mask("99/99/9999");
  });
  /* end_ch_mod */	
}
...
ajax.widgets.checkout.obj.prototype.disableCheckout = function() {
  ...
  /*----------------------------------------------------------------------------*
  	Additional Cooper's Hawk Functionality
  	Alert user when trying to click shipping, payment, or summary section 
	before completing the customer information section first.
  	Written by: Peter Juszczyk 2013
  *----------------------------------------------------------------------------*/
  
  /*$("#opc_shipping_payment, #opc_summary_li").on("click", function() {
	  var msg = 'Please enter your Profile and Address then click "Continue" below before ' + 
	  'selecting Shipping and Payment Method. If you already have an account please sign in.';
	  xAlert(msg,"","I");
	  return false;
  });*/ 
...
ajax.widgets.checkout.obj.prototype.enableCheckout = function() {
  ...
  *----------------------------------------------------------------------------*/
  ...
  /*$("#opc_shipping_payment, #opc_summary_li").off("click");*/
...
ajax.widgets.checkout.obj.prototype._callbackUpdateProfile = function(data) {
  ...
  if (data.status == 1) {
	...
	/*----------------------------------------------------------------------------*
		Additional Cooper's Hawk Functionality
		Updates totals whenever profile is changed.
		Written by: Peter Juszczyk 2013
	*----------------------------------------------------------------------------*/
...
ajax.widgets.checkout.obj.prototype._prepareCheckout = function() {
  ...
  /*----------------------------------------------------------------------------*
  	Additional Cooper's Hawk Functionality
	If the cart has alcoholic products in it verify the customers age.
		Alert the customer if they are not logged in or not 21 or older.
  	Written by: Peter Juszczyk 2013
  *----------------------------------------------------------------------------*/
  
  else {
	  var bday_string = $('#u_birthday').val();
	  var valid_date = true;
	  var msg = '';
	  ...
	  // if the cart has alcohol in it check that the customer is 21 or older
	  if ($('#cart_has_alcohol').val() == 'Y'){
		  var isValidAge = bday_string == "" ? false : validateAge(bday_string);
		  if (!isValidAge){
			var msg = 'There is a product containing alcohol in your shopping cart. You must be 21 or older to continue. Please update your information or edit your cart contents.';
	  ...
	  // if the provided birthday is not valid
	  if (!valid_date){
		  // disable checkout
		  this.hideWaitMessage = true;
          this.disableCheckout();
	  ...
	  var destination ="/ship_compliant_shop_client.php";
  
  	  doAjax("#compliance_form", destination, "checkComplianceSuccess", "", "", "post", false);
	  
	  // if order is not compliant
	  if ($('#is_compliant').val() == 'N'){
		  // disable checkout
		  this.hideWaitMessage = true;
          this.disableCheckout();
          this.hideWaitMessage = false;
